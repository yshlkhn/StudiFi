    ...
    const uploadHeaderFiles = async () => {
        ...
        for (const file of filesArray) {
            ...
            if (file.size > 10 * 1024 * 1024) {
                setMessage({
                    type: "error", text: `${file.name} exceeds 10MB limit`,
                });
            ...
            try {
                ...
                const { data, error } = await supabase.functions.invoke("convert-upload",
                    {
                        body: formData,
                    }
                );
                ...
                setMessage({
                    type: "error",
                    text: error.message || `Failed to upload ${file.name}`,
                });
